...
"""The main TTA model and related functions."""
  ...
  def __init__(self,
    ...
    with tf.variable_scope(scope, default_name="bert"):
      ...
      with tf.variable_scope("encoder"):
        ...
        self.all_encoder_layers = transformer_model_smsan(
...
def transformer_model_smsan(input_tensor_q,
                      input_tensor_kv=None,
                      attention_mask=None,
                      hidden_size=768,
                      num_hidden_layers=12,
                      num_attention_heads=12,
  ...
  This is similar to an implementation of the original Transformer encoder.
  See the original paper:
  https://arxiv.org/abs/1706.03762
  Also see:
  https://github.com/tensorflow/tensor2tensor/blob/master/tensor2tensor/models/transformer.py
  Args:
    input_tensor_q: float Tensor of shape [batch_size, seq_length, hidden_size].
    input_tensor_kv: float Tensor of shape [batch_size, seq_length, hidden_size].
  ...
  input_shape = get_shape_list(input_tensor_q, expected_rank=3)
  batch_size = input_shape[0]
  seq_length = input_shape[1]
  ...
  # help the optimizer.
  prev_output = reshape_to_matrix(input_tensor_q)
    
  if input_tensor_kv is not None:
    layer_input_kv = reshape_to_matrix(input_tensor_kv)

  all_layer_outputs = []
  for layer_idx in range(num_hidden_layers):
    with tf.variable_scope("layer_%d" % layer_idx):
      layer_input_q = prev_output
      if input_tensor_kv is None:
        layer_input_kv = layer_input_q
