...
import os
from model import modeling_roformer_nystromformer
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    model = modeling_roformer_nystromformer.BertModel(
        config=bert_config,
    ...
    if init_checkpoint:
      ...
      ) = modeling_roformer_nystromformer.get_assignment_map_from_checkpoint(tvars, init_checkpoint)
      if use_tpu:
...
def get_lm_output(config, input_tensor, output_weights, label_ids, label_mask):
  ...
  input_shape = modeling_roformer_nystromformer.get_shape_list(input_tensor, expected_rank=3)
  ...
  with tf.variable_scope("cls/predictions"):
    ...
    with tf.variable_scope("transform"):
      input_tensor = tf.layers.dense(
          ...
          activation=modeling_roformer_nystromformer.get_activation(config.hidden_act),
          kernel_initializer=modeling_roformer_nystromformer.create_initializer(config.initializer_range))
      input_tensor = modeling_roformer_nystromformer.layer_norm(input_tensor)
  ...
  with tf.variable_scope("cls/predictions"):
    ...
    with tf.variable_scope("transform"):
      input_tensor = tf.layers.dense(
          ...
          activation=modeling_roformer_nystromformer.get_activation(bert_config.hidden_act),
          kernel_initializer=modeling_roformer_nystromformer.create_initializer(
...
def gather_indexes(sequence_tensor, positions):
  ...
  sequence_shape = modeling_roformer_nystromformer.get_shape_list(sequence_tensor, expected_rank=3)
...
def main(_):
  ...
  bert_config = modeling_roformer_nystromformer.BertConfig.from_json_file(FLAGS.bert_config_file)
