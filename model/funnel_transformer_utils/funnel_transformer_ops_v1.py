...
def rel_pos_bias(q_head, pos_enc, d_model, n_head, d_head, initializer,
  ...
  print((q_head_r_1).get_shape(), 
    (enc_k_1).get_shape(), "==rel_pos_bias shape==", pos_bias.get_shape())
...
def rel_seg_bias(q_head, seg_mat, n_head, d_head, initializer, func_mask=None,
  ...
  print((q_head_s).get_shape(), 
    (seg_embed).get_shape(), "==rel_seg_bias shape==", seg_biases.get_shape())
...
def seg_id_to_mat(net_config, seg_q, seg_k):
  ...
  cls_mat = tf.logical_or(
      tf.expand_dims(tf.equal(seg_q, tf.constant([net_config.seg_id_cls], dtype=seg_q.dtype)), -1),
      tf.expand_dims(tf.equal(seg_k, tf.constant([net_config.seg_id_cls], dtype=seg_k.dtype)), -2))
