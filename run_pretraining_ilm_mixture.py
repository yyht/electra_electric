...
import os
from model import modeling_bert_unilm
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    ilm_input_mask = tf.cast(features['ilm_input_mask'], dtype=tf.int32)

    ilm_shape = modeling_bert_unilm.get_shape_list(ilm_input_ids)
    
    is_training = (mode == tf.estimator.ModeKeys.TRAIN)

    ilm_model = modeling_bert_unilm.BertModel(
        config=bert_config,
...
data_config.initial_ratio = 0.25
data_config.final_ratio = 0.25
...
def finetuning_input_fn_builder(
                     input_files,
                     max_seq_length,
                     max_predictions_per_seq,
                     real_max_length,
