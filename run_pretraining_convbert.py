  ...
  with tf.variable_scope("cls/predictions"):
    ...
    with tf.variable_scope("transform"):
      input_tensor = tf.layers.dense(
          ...
          activation=modeling_convbert.get_activation(config.hidden_act),
          kernel_initializer=modeling_convbert.create_initializer(config.initializer_range))
      input_tensor = modeling_convbert.layer_norm(input_tensor)
  ...
  with tf.variable_scope("cls/predictions", reuse=tf.AUTO_REUSE):
    ...
    with tf.variable_scope("transform"):
      input_tensor = tf.layers.dense(
          ...
          activation=modeling_convbert.get_activation(bert_config.hidden_act),
...
def gather_indexes(sequence_tensor, positions):
  ...
  sequence_shape = modeling_convbert.get_shape_list(sequence_tensor, expected_rank=3)
...
def main(_):
  ...
  bert_config = modeling_convbert.BertConfig.from_json_file(FLAGS.bert_config_file)
