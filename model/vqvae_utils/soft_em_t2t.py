...
class DiscreteBottleneck(object):
  ...
  def embedding_lookup(self, x, means):
    ...
    # x_means_hot_flat: [batch*T, num_blocks, block_v_size]
    # means: [num_blocks, block_v_size, block_dim]
    # x_means: [num_blocks, batch*T, block_dim]
  ...
  def bit_to_int(self, x_bit, num_bits, base=2):
    ...
    res = sum(x_labels)
    return tf.to_int32(tf.reshape(res, shape_list(x_bit)[:-1]))
