...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if init_checkpoint:
      ...
      ) = modeling_funnelformer.get_assignment_map_from_checkpoint(tvars, init_checkpoint)
      if use_tpu:
...
def get_lm_output(config, input_tensor, output_weights, label_ids, label_mask):
  ...
  input_shape = modeling_funnelformer.get_shape_list(input_tensor, expected_rank=3)
  ...
  with tf.variable_scope("cls/predictions"):
    ...
    with tf.variable_scope("transform"):
      input_tensor = tf.layers.dense(
          ...
          activation=modeling_funnelformer.get_activation(config.hidden_act),
          kernel_initializer=modeling_funnelformer.create_initializer(config.initializer_range))
...
def gather_indexes(sequence_tensor, positions):
  ...
  sequence_shape = modeling_funnelformer.get_shape_list(sequence_tensor, expected_rank=3)
...
def main(_):
  ...
  bert_config = modeling_funnelformer.ModelConfig.init_from_json(FLAGS.bert_config_file)
